...
若不一致就会引入 block recovery 过程进行恢复。下面是整个恢复处理流程的简要算法描述：
  ...
  4. 主导节点收集了所有节点上的 replica 信息后，就可以计算出最小的 block 长度。
  5. 主导节点向其他节点发起更新，将各自的 replica 更新为最小 block 长度，保持各节点 replica 长度一致。
...
如上图所示，pipeline 写入包括三个阶段
  ...
  Client 持续往 pipeline 发送 packet，在收到 packet ack 之前允许发送的 n 个 packet，n 就是 Client 的发送窗口大小（类似 TCP 滑动窗口）。
  3. __Close__：
...
#### 从 Data Streaming 错误中恢复
  ...
  1. 当某个 pipeline 中的 DataNode 检测到写入磁盘出错（可能是磁盘故障），它自动退出 pipeline，关闭相关的 TCP 连接。
  2. 当 Client 检测到 pipeline 有 DataNode 出错，先停止发送数据，并基于剩下的正常的 DataNode 重新构建 pipeline 再继续发送数据。
  3. Client 恢复发送数据后，从没有收到确认的数据开始重发，其中有些数据可能某些 DataNode 已经收到过了，则忽略不再重复存储直接传递到下游节点。
...
这里有策略是可配置的，称为 DataNode Replacement Policy upon Failure，包括下面几种情况：
  ...
  3. DEFAULT：默认根据副本数要求来决定，简单说配置的副本数为 3，如果坏了 2 个 DataNode，则会替换，否则不替换
  4. ALWAYS：总是替换


## 总结
本文讲述了 HDFS 异常处理与恢复的处理流程和细节，它确保 HDFS 在面对网络和节点错误的情况下保证数据写入的持久性和一致性。
读文本篇相信你会对 HDFS 内部的一些设计和工作状态有更深的认识，本文特地抽象的描述了一些设计具体算法的部分。
因为 HDFS 作为一个开源软件还在不断发展演变，具体算法代码实现总会变化，
但设计理念和 design for failure 的思考要点的持续性要长久的多，会更具参考价值。
